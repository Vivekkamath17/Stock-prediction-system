...
# Force stdout to UTF-8 to prevent cp1252 crash on Windows when printing emojis
...
def run_prediction_pipeline(stock_name: str, ticker: str, indian_api_key: str, news_api_key: str):
    ...
    You MUST output the scores for each headline in EXACTLY this strict format at the very end of your response:
    ...
    import re
    # This looks for patterns like "[Sentiment: Positive, Confidence: 0.9368]"
    matches = re.findall(r"\[Sentiment:\s*(Positive|Negative|Neutral),\s*Confidence:\s*([0-9.]+)\]", clean_text, re.IGNORECASE)
    ...
    if matches:
        ...
        return {
            ...
            "action": action,
            "newsStatus": "positive" if average_sentiment > 0 else "negative" if average_sentiment < 0 else "neutral"
        }
    else:
...
if __name__ == "__main__":
    ...
    TARGET_TICKER = sys.argv[1] if len(sys.argv) > 1 else "TMPV"
    TARGET_STOCK = sys.argv[2] if len(sys.argv) > 2 else "Tata Motors"
    
    # Provide your API keys
    MY_INDIAN_API_KEY = os.getenv("INDIAN_API_KEY") 
    MY_NEWS_API_KEY = os.getenv("NEWS_API_KEY")
    
    output = run_prediction_pipeline(TARGET_STOCK, TARGET_TICKER, MY_INDIAN_API_KEY, MY_NEWS_API_KEY)
    
    if output:
        print(f"\n---JSON_OUTPUT---{json.dumps(output)}---JSON_OUTPUT---")
